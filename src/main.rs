...
impl Db {
    ...
    fn read_schema(&self) -> rusqlite::Result<Vec<SchemaEntry>> {
        let mut sqlite_master = self.conn.prepare("SELECT type, name, tbl_name, sql FROM sqlite_master")?;
    ...
    fn read_schema_entry(&self, row: &Row) -> rusqlite::Result<SchemaEntry> {
        ...
        let column_names = tbl_info.query_map(NO_PARAMS, |row| row.get(1))?.collect::<Result<_, _>>()?;
        Ok(SchemaEntry {
            kind: row.get(0)?,
            name,
            tbl_name: row.get(2)?,
    ...
    fn read_table(&self, schema: &SchemaEntry) -> rusqlite::Result<TableReader> {
        Ok(TableReader {
            stmt: self.conn.prepare(&format!("SELECT * FROM {}", schema.name))?,
        })
    }
}
...
impl Sink for DirSink {
    ...
    fn open_table(&mut self, name: impl AsRef<str>) -> io::Result<FileTable> {
        Ok(FileTable { file: self.open_file(format!("data/table/{}.json", name.as_ref()))? })
    }
}
...
impl TableSink for FileTable {
    fn write_row(&mut self, row: &Row) -> io::Result<()> {
        ...
        let values: Vec<_> = (0..row.column_count()).map(|i| {
            match row.get_raw(i) {
